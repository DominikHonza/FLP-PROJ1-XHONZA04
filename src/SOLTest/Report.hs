...
-- * If a definition is missing → the test is ignored
--
-- * For each category:
--     * total points are summed from all tests in that category
--     * passed points are summed only for tests with result 'Passed'
--     * individual test results are stored in 'crTestResults'
-- == Result
--
-- A 'Map' from category name to 'CategoryReport', where each report contains:
--
...
-- Aggregates metrics about discovered, loaded, selected, and executed tests.
--
-- == Behavior
--
-- * If category results are available:
--     * histogram is computed using 'computeHistogram'
--     * passed tests are counted across all categories
--
-- * If category results are 'Nothing' (e.g. dry-run):
--     * histogram is empty (all bins set to 0)
--     * passed test count is 0
--
-- == Result
--
-- Returns a 'TestStats' record
--
-- AI DISCLAIMER:
-- AI Assisted: i was unable to comme up with this inner passedTests function
-- passedTest was generated by AI
computeStats ::
